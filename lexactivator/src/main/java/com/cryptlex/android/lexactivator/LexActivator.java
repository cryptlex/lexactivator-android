...
public class LexActivator {
    ...
    public static void SetProductFile(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.SetProductFile(filePath);
        if (LA_OK != status) {
    ...
    public static void SetProductData(String productData) throws LexActivatorException {
        int status;
        status = LexActivatorNative.SetProductData(productData);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
    ...
    */
    ...
    public static void SetCustomDeviceFingerprint(String fingerprint) throws LexActivatorException {
        int status;
        status = LexActivatorNative.SetCustomDeviceFingerprint(fingerprint);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
    ...
    public static void SetLicenseKey(String licenseKey) throws LexActivatorException {
        int status;
        status = LexActivatorNative.SetLicenseKey(licenseKey);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
    ...
    public static void SetTwoFactorAuthenticationCode(String twoFactorAuthenticationCode) throws LexActivatorException {
        int status;
        status = LexActivatorNative.SetTwoFactorAuthenticationCode(twoFactorAuthenticationCode);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
    ...
    public static int GetActivationLastSyncedDate() throws LexActivatorException {
        int status;
        IntByReference activationLastSyncedDate = new IntByReference(0);
        status = LexActivatorNative.GetActivationLastSyncedDate(activationLastSyncedDate);
        switch (status) {
        case LA_OK:
            return activationLastSyncedDate.getValue();
        case LA_FAIL:   
            return 0;
        default:
            throw new LexActivatorException(status);
    ...
    public static void CheckForReleaseUpdate(String platform, String version, String channel,
        ...
        if (privateReleaseCallback == null) {
            ...
            int status;
            status = LexActivatorNative.CheckForReleaseUpdate(platform, version, channel, privateReleaseCallback);
            if (LA_OK != status) {
                throw new LexActivatorException(status);
            }
    ...
    public static int ActivateLicenseOffline(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.ActivateLicenseOffline(filePath);
        switch (status) {
        case LA_OK:
            return LA_OK;
        case LA_EXPIRED:
    ...
    public static void GenerateOfflineActivationRequest(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.GenerateOfflineActivationRequest(filePath);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
    ...
    public static int GenerateOfflineDeactivationRequest(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.GenerateOfflineDeactivationRequest(filePath);
        switch (status) {
        case LA_OK:
            return LA_OK;
        case LA_FAIL:
    ...
    public static int ActivateTrialOffline(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.ActivateTrialOffline(filePath);
        switch (status) {
        case LA_OK:
            return LA_OK;
        case LA_TRIAL_EXPIRED:
    ...
    public static void GenerateOfflineTrialActivationRequest(String filePath) throws LexActivatorException {
        int status;
        status = LexActivatorNative.GenerateOfflineTrialActivationRequest(filePath);
        if (LA_OK != status) {
            throw new LexActivatorException(status);
        }
    }
